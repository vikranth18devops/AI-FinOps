...
## 📌 Step Overview
This step deploys the **PostgreSQL database** (`postgres:15-alpine`) inside the EKS cluster as a **Kubernetes StatefulSet** with a `10Gi` `PersistentVolumeClaim` (PVC) bound to Amazon EBS volume storage (`gp2` / `gp3`).
...
postgres-data-postgres-0   Bound    pvc-12345678-abcd-1234-abcd-1234567890ab   10Gi       RWO            gp2            1m
```
...
```
> Expected Output: Resolves to ClusterIP (e.g. `Server: 10.100.0.10 Address: 10.100.20.113`).
