# 07 - ArgoCD GitOps Continuous Deployment Setup on EKS
...
## 📌 Step Overview
Deploy the complete FinOps application stack (**React Frontend**, **FastAPI Backend**, **UI_Script Provisioner Studio**, and **In-Cluster PostgreSQL**) using an automated **GitOps continuous delivery loop** powered by **ArgoCD** on AWS EKS.
...
[GitHub Repo: vikranth18devops/AI-FinOps]
      ...
      [AWS EKS Cluster (finops namespace)]
```
...
```bash
# 1. Fetch Traefik LoadBalancer Hostname
export TRAEFIK_HOST=$(kubectl get svc -n ingress-traefik traefik -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
...
echo "  • Access URL: http://${TRAEFIK_HOST}/argocd/"
echo "  • Username:   admin"
