# AI-FinOps — PostgreSQL Database & StatefulSet Validation Guide (AWS EKS)

This guide explains how to connect to the **in-cluster PostgreSQL database** (`postgres-0`) running inside the **AWS EKS** cluster and query every table belonging to the AI Cloud Cost Detective microservices.

It is the **post-deploy companion** to `docs/aws/01-10` — once Phase 7 is green and pods are `Ready` (`1/1 Running`), every command below works out-of-the-box.

---

## How the Database is Set Up

AI-FinOps runs **1 PostgreSQL pod** (`postgres-0`) as a StatefulSet inside the `finops` namespace backed by AWS EBS Volumes (`gp3` storage class). Every microservice component (FastAPI Backend, UI_Script Studio, React Dashboard) points at **one central database called `cloud_cost_db`**.
...
> 💡 **A fresh cluster is NOT empty** — the database migration auto-seeds **2 historical remediation records** on first startup (`aws ec2 stop-instances...`, `aws ec2 delete-volume...`). Every other table initializes cleanly.

---

## Step 1: Connect to the Cluster + the Postgres Pod

First, make sure your `kubectl` context is pointed at your AWS EKS cluster:

```bash
# 1. Fetch EKS credentials
aws eks update-kubeconfig --region us-east-1 --name finops-eks-cluster
...
```bash
# Total dollars saved via automated remediations on EKS
kubectl exec -n finops postgres-0 -- \
  psql -U finopsadmin -d cloud_cost_db \
...
# Backup database to local file on AWS workstation
kubectl exec -n finops postgres-0 -- pg_dump -U finopsadmin cloud_cost_db > aws_finops_backup.sql
```
