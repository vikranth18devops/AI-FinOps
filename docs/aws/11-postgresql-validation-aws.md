# 11 - In-Cluster PostgreSQL StatefulSet Validation Guide (AWS EKS)

## 📌 Step Overview
This step covers validating database connectivity, schema creation, persistent volume storage integrity, and live record querying for the in-cluster **PostgreSQL StatefulSet** (`postgres-0`) running on **AWS EKS**.
...
# 2. Check PersistentVolumeClaim binding to AWS EBS
...
Connect directly to the PostgreSQL database inside the EKS pod using `kubectl exec`:
...
-- 2. Query executed fix commands and total dollars saved
...
async def test_db():
    ...
    print('✅ Connected successfully to in-cluster PostgreSQL on AWS EKS!')
