# 09 - Observability & Logging Stack on GCP GKE (Prometheus + Grafana + Loki + Tempo + Alertmanager)
...
<p align="left">
  ...
  <img src="https://img.shields.io/badge/Google_Cloud-GKE_Observability-4285F4?style=for-the-badge&logo=google-cloud&logoColor=white" />
</p>

## 📌 Step Overview
Deploy a production-grade observability and log aggregation stack to GCP GKE — **Prometheus** (metrics), **Grafana** (dashboards), **Alertmanager** (alerts), **Loki** (log store), **Tempo** (distributed tracing), and **OpenTelemetry Collector** (OTLP ingestion).
...
Verify all observability pods on GKE:
```bash
