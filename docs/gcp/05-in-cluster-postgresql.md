...
## 📌 Step Overview
This step deploys the **PostgreSQL database** (`postgres:15-alpine`) inside the GKE cluster as a **Kubernetes StatefulSet** with a `10Gi` `PersistentVolumeClaim` (PVC) bound to Google Persistent Disk storage (`standard-rwo`).
...
postgres-data-postgres-0   Bound    pvc-12345678-abcd-1234-abcd-1234567890ab   10Gi       RWO            standard-rwo   1m
```
...
```
> Expected Output: Resolves to ClusterIP (e.g. `Server: 10.96.0.10 Address: 10.96.20.113`).
